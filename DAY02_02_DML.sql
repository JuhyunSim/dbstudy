...
--1.DEPARTMENT_T 에서 부서(LOCATION)가 2인 부서원들의 지역을 '인천'으로 수정하시오
UPDATE DEPARTMENT_T
   SET LOCATION = '인천'
 WHERE DEPT_NO = 2;

--2. EMPLOYEE_T에서 부서번호(DEPART)가 2인 부서의 사원들의 기본급(SALARY)을 1-0%
UPDATE EMPLOYEE_T
   SET SALARY = SALARY * 1.1
 WHERE DEPART = 2;
  
-- 롤백 (이전 커밋으로 돌아감. 즉, 여기서는 2개의 UPDATE문이 취소됨)
ROLLBACK;
...
*/
-- 1. DEPARTMENT_T에서 부서번호(DEPT_NO)가 3인 부서를 삭제하시오.(부서번호가 3인 사원은 없다. 따라서 참조 무결성에 영향을 미치지 않음)
DELETE 
  FROM DEPARTMENT_T 
 WHERE DEPT_NO = 3;

-- 2. DEPARTMENT_T에서 부서번호(DEPT_NO)가 1과 4인 사원을 삭제하시오. 
DELETE
  FROM DEPARTMENT_T
 WHERE DEPT_NO IN(1,4); --WHERE DEPART IN(1, 4)

-- 3. DEPARTMENT_T에서 부서번호(DEPT_NO)가 2인 부서를 삭제하시오. 
--(부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 하지만 ON DELETE SET NULL 처리 되어 있어 DELETE 가능)
DELETE
  FROM DEPARTMENT_T
 WHERE DEPT_NO = 2;
