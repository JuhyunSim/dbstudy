/*
    ...
    4. DQL을 실행해도 데이터베이스에 변화는 발생하지 않는다. (검색기능이기 때문에)
    5. 형식
        SELECT 조회할 칼럼, 조회할 칼럼, ...
            FROM 테이블명  (FROM절 까지가 필수. 생략불가. 이 다음부터는 생략 가능)
            WHERE 조건식
            GROUP BY 그룹화할 칼럼, 그룹화할 칼럼, ...
            HAVING 그룹조건식 (GROUP BY와 같이 사용)
            ORDER BY 정렬할칼럼 ASC|DESC, 정렬할 칼럼 ASC|DESC   (오름차순인지, 내림차순인지)  
*/

--1. 부서 테이블의 모든 칼럼을 중복을 제거하여 조회하시오. 
SELECT DEPT_NO, DEPT_NAME, LOCATION
  FROM DEPARTMENT_T;
  
SELECT *  -- 모든 칼럼을 의미함(*), 실제 개발할 때는 사용 금지(전체적으로 시스템을 느려지게 할 수 있으므로 사용 금지)
  FROM DEPARTMENT_T;
...
SELECT LOCATION 
  FROM DEPARTMENT_T
 GROUP BY LOCATION; --같은 LOCATION은 그룹화해서(하나로 묶어서) 조회하라는 뜻
 ...
 /*
    ...
    4. 칼럼 IS NULL
    5. 칼럼 IS NOT NULL
    6. 칼럼 LIKE 값 (값이 포함된 데이터를 조회) EX. 포털검색 -> 특정 키워드 포함된 검색 조회됨 
 */
 --4. 부서 테이블에서 부서번호가 1인 부서 정보를 조회하시오. 
SELECT *
  FROM DEPARTMENT_T
 WHERE DEPT_NO = 1; -- DEPT_NO가 PK(UNIQUE)이므로 단일 결과값

--5. 부서 테이블에서 지역이 '서울'인 부서 정보를 조회하시오. 
SELECT *
  FROM DEPARTMENT_T
 WHERE LOCATION = '서울'; -- LOCATION이 UNIQUE가 아니므로 다중 결과값
...
--7. 사원 테이블에서 기본급이 2000000~3000000인 사원을 조회하시오. 
SELECT *
  FROM EMPLOYEE_T
