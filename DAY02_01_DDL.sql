...
CREATE TABLE CUSTOMER_T(
    ...
    CONSTRAINT  PK_CUSTOMER /*제약조건명*/         PRIMARY KEY(CUST_NO), 
    CONSTRAINT  FK_CUSTOMER_BANK /*제약조건명*/    FOREIGN KEY(BANK_CODE) REFERENCES BANK_T(BANK_CODE)
    --외래코드가 있을 때, 외래코드를 먼저 입력해야 다른 데이터를 입력할 수 있음.(NULL)값을 넣어주고 입력해야함.          
);
...
/*
    ...
    5. 테이블 이름바꾸기      : ALTER TABLE 테이블명 RENAME TO 신규테이블명
*/

-- 1. BANK_T에 연락처 칼럼(BANK_TEL) 추가
...
ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR2(30 BYTE)  CHECK(GRADE IN ('VIP', 'GOLD', 'SILVER', 'BRONZE'));
                                                -- CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = SILVER' OR GRADE = 'BRONZE');
                                                -- IN 연산자 사용을 권장

-- 6. CUSTOMER_T의 테이블명을 CUST_T로 수정하시오. 

ALTER TABLE CUSTOMER_T RENAME TO CUST_T;

-- 7. CUST_T의 고객명(CUST_NAME) 칼럼의 필수 제약조건을 없애시오.

ALTER TABLE CUST_T MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL;
                 -- CREATE와 달리 ALTER에서는 필수제약조건을 없앨 때, NULL을 적어주어야 함(생략 불가)    
...
/*
    ...
    2. FK
        1) 추가      : ALTER TABLE 자식 테이블명 ADD CONSTRAINT     제약조건명 FOREIGN KEY(칼럼명) REFERENCES 부모테이블명(참조할 칼럼명)
...
--외래키가 동작하지 않게 됨(부모키가 없어도 입력할 수 있게 됨)
